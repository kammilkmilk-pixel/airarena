...
function selectTeam(teamId) {
    if (GameContext.isAnimating() || GameContext.isReplayMode()) return;
...
function zoomToSelf() {
    const active = GameContext.getActiveTeam();
    if (!active || !active.wrapper || active.isDestroyed || GameContext.isReplayMode()) return;
    let backOffset = new THREE.Vector3(0, 3.5, -10).applyQuaternion(active.wrapper.quaternion);
    let targetCamPos = active.wrapper.position.clone().add(backOffset);
    ...
    function doLerp(now) {
        ...
        camera.position.lerpVectors(startCamPos, targetCamPos, elapsed); controls.target.lerpVectors(startTarget, active.wrapper.position, elapsed);
        if (elapsed < 1 && !GameContext.isReplayMode()) requestAnimationFrame(doLerp);
    }
                ...
                if (t.pylons && currentLog) {
                    t.pylons.forEach(p => {
                        ...
                        if (explodeFrame !== undefined && trackIdx >= explodeFrame && !p.hasBoomedThisTurn) {
                            ...
                            if (isSelfDestruct && !GameContext.isReplayMode()) { let banner = document.getElementById('phase-banner'); if (banner) { banner.innerHTML = `<span style="font-size:24px; color:#ff5500; font-weight:bold; text-shadow: 2px 2px 4px #000;">💥 飛彈達最大航程自毀</span>`; banner.style.opacity = '1'; setTimeout(() => banner.style.opacity = '0', 1500); } } 
