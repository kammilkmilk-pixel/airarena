...
function toggleReadyState(teamId) {
    ...
    updateDashboardUI(teams[uiCurrentTeamId()]); // 觸發儀表更新
    ...
    } else if (nextReady && uiCurrentTeamId() === teamId) { 
        // 只有自己待命，自動切換畫面給對方操作
        if(window.selectTeam) window.selectTeam(oppId); 
    }
}

let btnEngageRed = document.getElementById('btn-engage-red');
if(btnEngageRed) btnEngageRed.addEventListener('click', () => toggleReadyState('red'));

let btnEngageBlue = document.getElementById('btn-engage-blue');
if(btnEngageBlue) btnEngageBlue.addEventListener('click', () => toggleReadyState('blue'));
