...
scene.fog = new THREE.Fog(0x87b8e8, 120, 620);
...
const hemiLight = new THREE.HemisphereLight(0xb9dcff, 0x3b3328, 0.8);
scene.add(hemiLight);

const battlefieldCenter = new THREE.Vector3(10, 0, 20);
const sunDirection = new THREE.Vector3(-0.55, 0.82, -0.35).normalize();
const dirLight = new THREE.DirectionalLight(0xfff1c7, 1.25);
...
const groundMat = new THREE.MeshStandardMaterial({ color: 0x48503b, roughness: 0.95, metalness: 0.0 });
...
function initCityMapModel() {
    ...
    gltfLoader.load(
        ...
        function (error) {
            ...
            window.AssetFallbacks.buildProceduralCity(scene, obstacles);
        }
    );
}
